...
public class OrdersModel {
    ...
    private boolean increaseInventory(String menuItemID, double quantityOfOrder) throws SQLException, ClassNotFoundException {
    ...
    public boolean updateOrder(OrdersDto orderDTO, ArrayList<OrderItemsDto> newOrderItemsDtos, ArrayList<PaymentsDto> paymentsDtos) throws SQLException, ClassNotFoundException {
        Connection connection = DBConnection.getInstance().getConnection();
        try {
            ...
            ArrayList<OrderItemsDto> existingOrderItemsDtos = new ArrayList<>();
            while (existingItemsResult.next()) {
                ...
                existingOrderItemsDtos.add(new OrderItemsDto(orderDTO.getOrderID(), menuItemID, quantity, 0.0));
            }

            // Step 2: Identify items to delete and restore inventory for them
            for (OrderItemsDto existingItem : existingOrderItemsDtos) {
                boolean itemExistsInNewList = false;
                for (OrderItemsDto newItem : newOrderItemsDtos) {
    ...
    public ArrayList<OrderItemsDto> getOrderItemsByOrderId(String orderId) throws SQLException, ClassNotFoundException {
        Connection connection = DBConnection.getInstance().getConnection();
